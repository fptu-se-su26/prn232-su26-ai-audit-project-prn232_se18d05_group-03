...
| Testing |  |  |  | x | 2026-07-25/26 (DE190123): feature completion matrix (40 features) + integration audit 4 pass; end-to-end test Flow 1-4 (31 bước), 6 edge case, 5 performance check, 3 regression check qua browser thật + SQL trực tiếp |
...
| Mục đích sử dụng    | Thiết kế lại UI hiện đại: bản đồ giường 2D/3D, design system, redesign 6 trang |
| Phần việc liên quan | Member 3 – F1 BedMap UI, global theme                                          |
| Mức độ sử dụng      | Viết code toàn bộ                                                              |

#### 9.1. Prompt đã sử dụng

```text
UI hiện tại chưa được đẹp. Ở F1 muốn làm một cái sơ đồ 2D và 3D có thể chuyển đổi được.
Bệnh viện có nhiều phòng và nhiều khu nên hãy làm có thể chọn được. Giường bệnh sẽ hiện ra
những status có sẵn theo màu, giường bệnh sẽ đổi màu theo trạng thái. Những phần khác làm
giao diện hiện đại như các bệnh viện lớn, ví dụ Long Châu.
```

#### 9.2. Kết quả AI gợi ý

```text
1. Backend: thêm Floor, PositionX, PositionY vào Bed entity; thêm BedPositionDto; thêm
   PATCH /api/beds/{id}/position (Admin only); restrict Create/Delete beds đến Admin;
   JsonStringEnumConverter để API trả string enum. EF migration AddBedSpatialFields.

2. Seed: DbInitializer.SeedBedsAsync() – 36 giường với vị trí X/Y đặt sẵn cho 2 khoa,
   2 Ward/khoa, 3 phòng/ward, 3 giường/phòng, trạng thái đa dạng.

3. Web global theme: app.css viết lại với CSS custom properties (brand blue, teal,
   status colours), Inter font, card/button/badge/table override. MainLayout + NavMenu
   redesign thành sidebar tối navy với Material Symbols icon + label.

4. F1 BedMap: rewrite thành floor plan tương tác – stage có tỷ lệ 16:9, giường tile
   đặt tuyệt đối tại PositionX/Y%, room outline từ bounding-box của các giường cùng
   RoomNumber, toggle 2D/3D bằng CSS perspective + rotateX/Z isometric, edit mode
   (Admin) kéo-thả via floorplan.js + JS interop [JSInvokable] OnBedDropped,
   form thêm/xoá giường, drawer chi tiết + đổi trạng thái (Admin/Nurse), modal
   nhập viện, modal chuyển giường.

5. Home: dashboard card với hero greeting + 4 feature tiles đẹp.
6. Login: split-screen brand panel + form card với demo accounts.
7. F2/F3/F4: giữ logic API, cải thiện markup với icon Material Symbols, form
   có nền tách biệt, table header uppercase muted, lab dropzone, discharge invoice.
...
| ------------------- | ------------------------------------------------------------------------------ |
| Ngày sử dụng        | 01/07/2026                                                                     |
| Công cụ AI          | Claude Code (claude-sonnet-4-6 / claude-opus-4-8)                              |
| Mục đích sử dụng    | Nâng cấp sơ đồ giường F1: zoom/pan, bản 2D kiểu CAD, bản 3D thật bằng Three.js |
| Phần việc liên quan | Member 3 – F1 BedMap 2D/3D                                                     |
| Mức độ sử dụng      | Viết code toàn bộ                                                              |

#### 10.1. Prompt đã sử dụng

```text
- Thêm chức năng phóng to thu nhỏ + bàn tay kéo di chuyển (vì phóng to chỉ về 1 hướng).
- Sửa lại giao diện map 2D trực quan hơn (kiểu bản vẽ CAD, tô màu theo khu).
- Muốn 3D giống ảnh Google Maps (khối nổi thật).
```

#### 10.2. Kết quả AI gợi ý

```text
1. Zoom/pan (2D): nút overlay +/−/% trong khung sơ đồ; StageTransform gộp pan + zoom +
   transform; kéo tay (grab) qua floorplan.js initPan + [JSInvokable] SetPan; đổi
   transform-origin về center để phóng đều 4 phía.

2. 2D kiểu CAD: WardColor() palette ổn định theo khu; phòng tường đôi nét + nền tô nhạt
   color-mix theo màu khu; hành lang vân hatch; title block "SƠ ĐỒ TẦNG"; legend thêm
   chip màu khu; nền lưới graph-paper.

3. 3D thật (Three.js): vendor three@0.160.1 + OrbitControls vào wwwroot/lib/three,
   import map trong App.razor (no build step). floorplan3d.js (ES module) dựng scene:
   ground + phòng khối đùn (slab + 4 tường) + giường khối màu theo trạng thái + sprite
   nhãn + ánh sáng đổ bóng + OrbitControls (xoay/zoom/pan). Blazor nạp module qua JS
   isolation, render từ payload beds/rooms/corridor; click giường → Raycaster →
   [JSInvokable] SelectBedFromJs mở drawer cũ; đổi trạng thái → updateBed recolor live;
   DisposeAsync dọn WebGL context/RAF khi rời 3D.
...
| ------------------- | ----------------------------------------------------------------- |
| Ngày sử dụng        | 01/07/2026                                                        |
| Công cụ AI          | Claude Code (claude-opus-4-8 / claude-sonnet-5)                   |
| Mục đích sử dụng    | Floor thật cho từng tầng + drill-down Tầng→Phòng→Giường (2D & 3D) |
| Phần việc liên quan | Member 3 – F1 BedMap seed đa tầng + layout engine + drill-down    |
| Mức độ sử dụng      | Viết code toàn bộ                                                 |

#### 11.1. Prompt đã sử dụng

```text
Layout một tầng như hiện tại chưa đúng, tôi muốn nó là một floor thật cho từng tầng,
xong ấn vào chọn phòng rồi chọn giường. Lấy bệnh viện nào cũng được.
(Chọn: áp dụng cả 2D và 3D; layout kết hợp template + tự sinh; seed nhiều tầng thực tế.)
```

#### 11.2. Kết quả AI gợi ý

```text
1. Seed đa tầng: DbInitializer.SeedBedsAsync mỗi khoa 3 tầng × 2 khu × 3 phòng × 3 giường,
   PositionX/Y=null (layout tự sinh). seed_hospital.sql regenerate 648 giường (12 khoa × 3
   tầng), PositionX/Y=NULL — sinh bằng script throwaway.

2. Layout engine C# ComputeFloorLayout(): nhóm giường theo phòng, xếp double-loaded corridor
   (hàng trên + hàng dưới quanh hành lang giữa), trạm y tá + thang máy; dùng chung cho 2D và
   3D (Build3dPayload). Bỏ RoomOutlines/CorridorBounds/WardLabels dựa trên toạ độ.

3. Drill-down: _selRoom (null=overview / set=phòng). 2D overview hiện phòng bấm được + badge
   occupied/total; bấm phòng → grid giường (.fp-bed-grid) → bấm giường → drawer cũ. 3D:
   click phòng (raycast) → SelectRoomFromJs + focusRoom bay camera vào phòng, hiện giường;
   exitRoom quay lại; updateBed recolor live; nút Back.

4. Retire editor toạ độ: bỏ kéo-thả bed trong floorplan.js (giữ initPan), bỏ input X/Y +
   SavePosition + OnBedDropped. Giữ zoom/pan, thêm/xoá giường, đổi trạng thái, nhập viện,
   chuyển khoa.

Build: 0 errors, 0 warnings (cả solution).
```
...
| Ngày sử dụng        | 01/07/2026                                               |
| Công cụ AI          | Claude Code (claude-opus-4-8 / claude-sonnet-5)          |
| Mục đích sử dụng    | Nâng cấp UI 3D lên mức realistic (post-processing + IBL) |
| Phần việc liên quan | Member 3 – F1 BedMap scene 3D                            |
| Mức độ sử dụng      | Viết code toàn bộ                                        |

#### 12.1. Prompt đã sử dụng

```text
Oke rồi, nhưng có thể nâng cấp UI 3D hơn hay làm sao không.
(Chọn: mức realistic tối đa; thêm tự xoay overview, tường kính mờ, nhãn HTML sắc nét,
phông giường theo trạng thái.)
```

#### 12.2. Kết quả AI gợi ý

```text
1. Vendor thêm addon three@0.160.1 (curl unpkg, giữ cây thư mục examples/jsm):
   RoomEnvironment, RoundedBoxGeometry, CSS2DRenderer + bộ postprocessing
   (EffectComposer, RenderPass, ShaderPass, MaskPass, OutputPass, SSAOPass,
   UnrealBloomPass, OutlinePass) + shaders (Copy/Output/SSAO/LuminosityHighPass) +
   math/SimplexNoise. node --check toàn bộ, kiểm dependency closure đủ.

2. floorplan3d.js viết lại: ACESFilmic tone mapping + sRGB + PCFSoft; ánh sáng IBL
   qua PMREMGenerator + RoomEnvironment; RoundedBoxGeometry cho phòng/giường; tường
   kính mờ (transparent, depthWrite=false) để thấy giường; giường chi tiết (khung +
   nệm + chăn màu theo trạng thái + gối + đầu giường); nhãn HTML CSS2DRenderer thay
   sprite; nền gradient + fog.

3. Post-processing: EffectComposer = RenderPass -> SSAO -> UnrealBloom (nhẹ) ->
   OutlinePass (hover/chọn) -> OutputPass; bọc try/catch fallback về renderer.render.

4. Tương tác: giường dựng sẵn trong mọi phòng (nhìn qua kính); hover raycast ->
   OutlinePass highlight; click giường -> SelectBedFromJs; click phòng -> bay camera
   + SelectRoomFromJs; auto-rotate ở overview, tắt khi kéo/khi vào phòng, bật lại khi
   Back; dispose dọn composer/env/labels/WebGL. C#/Blazor gần như không đổi.

Build: 0 errors, 0 warnings. node --check tất cả JS OK.
```
...
| Ngày sử dụng        | 01/07/2026                                                           |
| Công cụ AI          | Claude Code (claude-opus-4-8 / claude-sonnet-5)                      |
| Mục đích sử dụng    | Thêm nội thất phòng 3D thực tế (giường sát tường, rèm, thiết bị, WC) |
| Phần việc liên quan | Member 3 – F1 scene 3D (floorplan3d.js)                              |
| Mức độ sử dụng      | Viết code toàn bộ                                                    |

#### 13.1. Prompt đã sử dụng

```text
Trên thực tế giường bệnh nằm sát, tường có rèm, các máy chuyên ngành, nhà vệ sinh có rèm...
có thể thêm vào không? (Chọn cả 4: giường sát tường, rèm riêng tư mỗi giường,
thiết bị y tế cạnh giường, buồng vệ sinh trong phòng.)
```

#### 13.2. Kết quả AI gợi ý

```text
Chỉ sửa floorplan3d.js (không đụng backend/data/C#):
1. Giường xếp sát tường ngoài: thay cách rải lưới -> giường dàn 1 hàng dọc tường
   đối diện hành lang, đầu áp tường, gần nhau, xoay rotation.y theo hướng tường
   (phòng trên head -z, phòng dưới head +z).
2. makeCurtain: ray trần + tấm rèm bán trong suốt (teal) buông phía lối đi + đầu
   chân giường, depthWrite=false, renderOrder thấp hơn kính.
3. makeEquipment: monitor (cột + màn phát sáng emissive) + giá truyền dịch IV
   (cột + túi dịch mờ) + tủ đầu giường, thêm vào bed group (xoay theo giường).
4. makeBathroom: buồng WC ở góc phòng phía hành lang+phải (vách ngăn + rèm cửa
   bán trong suốt + sàn lát + bồn cầu + lavabo + nhãn WC), userData.roomNumber để
   click vẫn vào phòng; giường né footprint WC (ở đầu tường đối diện).
5. 2D: thêm chip "WC" ở header room-detail.
dispose() tự dọn vì mọi mesh add vào room group.

Build: 0 errors, 0 warnings. node --check JS OK.
```
...
| Ngày sử dụng        | 02/07/2026                                                                       |
| Công cụ AI          | Claude Code (claude-opus-4-8 / claude-sonnet-5)                                  |
| Mục đích sử dụng    | Loại phòng theo sức chứa (6/3/2/1) + đặt tên + bố cục giường 2 tường + trang trí |
| Phần việc liên quan | Member 3 – F1 seed + layout + scene 3D                                           |
| Mức độ sử dụng      | Viết code toàn bộ                                                                |

#### 14.1. Prompt đã sử dụng

```text
Chia thành nhiều loại phòng nằm, ví dụ phòng 6, 3, 2, 1. Được đặt tên.
Bệnh viện thường làm phòng nhiều giường về hai phía tường trái/phải, đầu hướng vào tường.
Trang trí thêm cho từng phòng (chọn: cửa+bảng tên, cửa sổ tường ngoài, đèn+bảng tên đầu
giường, thảm sàn+cây cảnh). Tên phòng tự suy theo số giường (không đổi DB).
```

#### 14.2. Kết quả AI gợi ý

```text
1. Seed đa dạng sức chứa: mỗi khu 4 phòng [6,3,2,1] giường (DbInitializer + regenerate
   seed_hospital.sql ~864 giường, PositionX/Y=NULL). Không migration.
2. RoomTypeName(beds): 1->Phòng đơn (VIP), 2->Phòng đôi, 3->Phòng 3 giường,
   6+->Phòng thường (n giường). Hiện ở card 2D, header room-detail, tag 3D.
3. Layout engine: PlaceRow cho chiều rộng phòng tỉ lệ số giường (weight clamp 2..6) ->
   phòng 6 to, phòng đơn nhỏ.
4. 3D bố cục kiểu khoa nội trú: giường dọc HAI tường trái (rotY=+90°) và phải (rotY=-90°),
   đầu áp tường, chân vào lối đi giữa; chia leftCount=ceil(n/2)/rightCount; giường tường
   phải né góc WC (reserve wcSq); nhãn số giường gắn tường đầu giường.
5. Trang trí mỗi phòng: makeDoor (khung cửa + userData để click vào phòng) trên tường
   hành lang lệch khỏi WC; makeWindow (kính mờ + khung) tường ngoài; đèn trần phát sáng
   trên mỗi giường; thảm sàn giữa lối đi; makePlant (chậu + tán lá) ở góc trống.
Mọi mesh add vào room group -> dispose() tự dọn.

Build: 0 errors, 0 warnings (cả solution). node --check JS OK.
```
...
| Mục đích sử dụng    | Verify end-to-end các chức năng F1 + tìm và sửa bug Nhập viện 500 |
| Phần việc liên quan | Member 3 – F1 admit/transfer/discharge                            |
| Mức độ sử dụng      | Kiểm thử tự động + viết code sửa lỗi                              |

#### 15.1. Prompt đã sử dụng

```text
Check lại các chức năng đã hoạt động được chưa, đúng chưa.
(Sau khi phát hiện bug: chọn Cách B — sửa trong endpoint admit của member 3,
không đụng controller của thành viên khác.)
```

#### 15.2. Kết quả AI gợi ý

```text
1. Verify tự động: API bằng curl+JWT (login, bed-map 3 tầng × 8 phòng × [6,3,2,1],
   status string-enum); UI bằng headless Chrome (playwright-core + Chrome hệ thống):
   login → F1 → overview 2D (tên loại phòng, kích thước theo sức chứa) → drill-down
   phòng → drawer → đổi trạng thái live → đổi tầng → 3D render (kính, 3+3, nhãn CSS2D,
   console sạch) → toggle 2D/3D lặp. Probe: Doctor 403, no-token 401, bed bogus 404,
   giường OutOfService ẩn nút Nhập viện.

2. Phát hiện bug: Nhập viện từ UI → "Lỗi 500". Nguyên nhân: InpatientAdmission.PatientId
   FK tới PatientProfiles.Id nhưng dropdown UI gửi UserAccount.Id; seed SQL tạo profile
   bằng NEWID() → FK violation. GET /api/patients (list) không tồn tại (405).

3. Fix (Cách B — gọn trong code member 3): endpoint admit resolve PatientId theo
   PatientProfile.Id HOẶC UserAccountId (FirstOrDefaultAsync), 404 rõ ràng nếu không có
   profile; dùng profile.Id khi tạo admission. Không đổi DTO/route/schema.

4. Verify lại: admit(UserAccount.Id) → 200 + giường Occupied; transfer sang khoa khác →
   giường cũ Cleaning, giường mới Occupied; discharge → invoice (500k/ngày) + giường
   Cleaning; admit từ UI hết lỗi. Dọn dữ liệu test về trạng thái ban đầu.
...
| Công cụ AI          | Claude Code (claude-sonnet-5)                                                       |
| Mục đích sử dụng    | Xây Feature 4 – Telemedicine (video call) từ đầu, sửa các bug lộ ra qua test sống trên trình duyệt |
| Phần việc liên quan | Backend (SignalR Hub) / Frontend (Blazor, JS interop WebRTC) / Git workflow          |
| Mức độ sử dụng      | Viết code toàn bộ + tự phát hiện & fix bug qua test thực tế, không chỉ đọc code tĩnh |
...
1. Tạo `TelemedicineHub` (SignalR) làm relay tín hiệu offer/answer/ICE giữa 2 circuit Blazor
   Server (bác sĩ + bệnh nhân) trong cùng phòng khám tele — không mang PHI, chỉ relay
   SDP/ICE, room id là GUID không đoán được nên đủ làm access control, không cần
   [Authorize] riêng.
2. Viết `telemedicine.js` (WebRTC interop qua JSInterop: getUserMedia, RTCPeerConnection, tạo
   offer/answer, add ICE candidate) và trang mới `/telemedicine/{RoomId}`: video local/remote,
   nút mute/tắt cam/kết thúc, panel riêng cho bác sĩ để ghi chú và mở đơn thuốc điện tử.
3. Bọc lời gọi `startLocalVideo` trong try/catch riêng — getUserMedia ném lỗi khi trình duyệt
   không có camera/mic hoặc người dùng từ chối quyền; nếu để lỗi này văng ra ngoài thì sẽ làm
   sập toàn bộ circuit Blazor Server (ngắt kết nối cả trang), không chỉ mỗi khung video. Tách
   biến `_mediaError` khỏi `_error` để phần ghi chú/đơn thuốc (phía bác sĩ) vẫn dùng được bình
   thường dù không có camera.
4. Hệ thống chưa có cơ chế đẩy thông báo (push) sẵn, nên thêm polling 10 giây ở trang
   Appointments của bệnh nhân (`GetPendingCallForPatient`, lọc phiên tele đã tạo nhưng chưa
   `StartedAt`) để hiện modal "có cuộc gọi đến", cho bệnh nhân bấm vào tham gia; tự động gọi
   `CompleteOutpatientVisit` khi cuộc gọi kết thúc.
5. Thêm `NotifyCallEnded` vào Hub. Vì `Clients.OthersInGroup` chỉ còn với thành viên vẫn nằm
   trong group, lệnh gửi này phải đứng trước `LeaveRoom` trong `EndCall()` chứ không phải sau
   — nếu không bên kết thúc cuộc gọi đã rời group trước khi tín hiệu kịp gửi, bên còn lại sẽ
   không bao giờ nhận được và đứng hình vô thời hạn.
```

#### 19.3. Phần sinh viên/nhóm đã sử dụng từ AI

```text
Áp dụng toàn bộ sau khi xác nhận `dotnet build` sạch (0 Warning/0 Error) và tự test sống bằng
2 tab trình duyệt (tài khoản bác sĩ + bệnh nhân) qua từng bug: từ chối quyền camera không làm
sập trang, bệnh nhân nhận được thông báo cuộc gọi đến trong vòng 10 giây, kết thúc cuộc gọi ở
1 bên thì bên còn lại thoát ngay thay vì đứng hình.
```

#### 19.4. Phần sinh viên/nhóm tự chỉnh sửa hoặc cải tiến

```text
- Tự phát hiện lỗi crash camera qua test thật (chủ động từ chối quyền camera trên trình
  duyệt) chứ không đợi đọc lại code, yêu cầu AI sửa ngay khi thấy trang trắng.
- Chấp nhận đánh đổi polling 10 giây (độ trễ tối đa 10s) thay vì thiết kế lại toàn bộ cơ chế
  push notification, vì hệ thống chưa có hub cho việc này và không muốn mở rộng phạm vi công
  việc quá xa mục tiêu chính (video call).
- Yêu cầu xác nhận thứ tự gửi `NotifyCallEnded` trước `LeaveRoom` bằng lời giải thích rõ ràng
  (`OthersInGroup` không còn thấy người đã rời group) trước khi chấp nhận fix.
```
...
| Loại minh chứng   | Nội dung                                                                                     |
| ----------------- | --------------------------------------------------------------------------------------------- |
| Link commit       | `bb5706d` trên branch `feature/de190123-telemedicine`                                        |
| File liên quan    | `src/Mediconnect.Web/Hubs/TelemedicineHub.cs`; `src/Mediconnect.Web/wwwroot/js/telemedicine.js`; `src/Mediconnect.Web/Components/Pages/Telemedicine.razor`; `src/Mediconnect.Web/Components/Pages/Appointments.razor`; `src/Mediconnect.Web/Components/Pages/Appointments.razor.css`; `src/Mediconnect.Web/Program.cs`; `src/Mediconnect.Web/Mediconnect.Web.csproj`; `src/Mediconnect.Web/Components/App.razor`; `src/Mediconnect.Web/Components/Pages/ClinicDashboard.razor`; `src/Mediconnect.Web/Services/ApiClient.cs` |
| Screenshot        |                                                                                                |
| Kết quả chạy/test | `dotnet build`: 0 Warning/0 Error sau mỗi bước; test sống 2 tab trình duyệt (doctor@mediconnect.local / patient@mediconnect.local): gọi video 2 chiều thành công, từ chối quyền camera không sập trang, thông báo cuộc gọi đến hiện đúng cho bệnh nhân, kết thúc cuộc gọi đồng bộ ở cả 2 bên |
| Link video demo   |                                                                                                |
| Ghi chú khác      | `Program.cs`, `Mediconnect.Web.csproj`, `App.razor`, `ClinicDashboard.razor`, `Appointments.razor`, `ApiClient.cs` vốn do teammate `minhPJ2812` viết — thay đổi lần này chỉ thêm mới (SignalR registration, script tag, nút bấm, polling, API method), không sửa logic có sẵn của các file này |

#### 19.6. Nhận xét cá nhân/nhóm

```text
Lỗi crash camera là ví dụ rõ cho việc một lỗi JS tưởng chừng nhỏ (permission denied) có thể
lan ra ngoài phạm vi tưởng tượng ban đầu trong mô hình Blazor Server — vì toàn bộ trang chạy
trên 1 circuit dùng chung kết nối SignalR, một exception không bắt ở JSInterop có thể ngắt cả
circuit chứ không dừng lại ở component gây lỗi. Việc test bằng cách chủ động từ chối quyền
camera (không phải chỉ đọc code) là cách duy nhất phát hiện được lỗi này trước khi nó xảy ra
với người dùng thật.
```
...
| Công cụ AI          | Claude Code (claude-sonnet-5)                                                       |
| Mục đích sử dụng    | Fix UI đơn thuốc điện tử bị chật khi nhúng cạnh khung video call                    |
...
```text
`EPrescriptionPanel.razor` vốn dùng Bootstrap `col-*`/`row` — cơ chế này chia layout theo bề
rộng viewport toàn trang (breakpoint sm/md/lg), không biết gì về bề rộng thật của khung chứa
nó. Khi nhúng panel vào cột hẹp (~360px) cạnh video call, Bootstrap vẫn tính layout theo
viewport rộng của toàn màn hình nên các trường bị chật, không co giãn đúng. Thay toàn bộ class
Bootstrap layout bằng CSS container-query riêng trong file mới `EPrescriptionPanel.razor.css`
— layout tự tính theo bề rộng khung chứa thực tế thay vì viewport. Không đổi hành vi, chỉ đổi
cách chia layout.
```
...
```text
Áp dụng sau khi xác nhận `dotnet build` sạch và xem trực quan qua browser ở cả 2 nơi dùng
panel: nhúng cạnh video (360px) và dùng độc lập ở trang Outpatient Record cũ (rộng hơn).
```

#### 20.4. Phần sinh viên/nhóm tự chỉnh sửa hoặc cải tiến

```text
Yêu cầu xác nhận rõ đây chỉ là thay đổi layout, không đổi hành vi/logic, trước khi đồng ý sửa
— vì đây là file dùng chung giữa 2 trang (Outpatient Record cũ và Telemedicine mới), lo ngại
sửa nhầm ảnh hưởng ngược lại trang cũ.
```
...
| Loại minh chứng   | Nội dung                                                                                     |
| ----------------- | --------------------------------------------------------------------------------------------- |
| Link commit       | `a7a1472` trên branch `feature/de190123-telemedicine`                                        |
...
| Screenshot        |                                                                                                |
| Kết quả chạy/test | `dotnet build`: 0 Warning/0 Error; kiểm trực quan qua browser ở cả 2 chỗ dùng panel, không phát hiện lệch layout mới |
| Link video demo   |                                                                                                |
| Ghi chú khác      | `EPrescriptionPanel.razor` vốn do teammate `minhPJ2812` viết — thay đổi lần này thay class layout có sẵn (không phải chỉ thêm mới) |

#### 20.6. Nhận xét cá nhân/nhóm

```text
Bug layout kiểu này (Bootstrap viewport-based col-* trong khung nhúng hẹp) rất dễ bị bỏ qua
khi review code tĩnh vì class trông vẫn "đúng chuẩn Bootstrap" — chỉ lộ ra khi thực sự nhìn
trang chạy trong đúng bối cảnh nhúng hẹp, không phải màn hình rộng lúc code lần đầu.
```
...
| Ngày sử dụng        | 23/07/2026                                                                           |
| Công cụ AI          | Claude Code (claude-sonnet-5)                                                       |
| Mục đích sử dụng    | Thêm ghi Triệu chứng & Chẩn đoán ICD-10 ngay trong lúc gọi video tele                |
...
```text
Trang Telemedicine (Lần sử dụng AI số 19) ban đầu chỉ có ô ghi chú tự do + nút mở đơn thuốc,
không có chỗ ghi triệu chứng/mã ICD-10 như luồng khám vãng lai đã có sẵn ở
`OutpatientRecord`. Thêm card "Triệu chứng & Chẩn đoán" vào panel bác sĩ: ô lý do khám chính,
ô mô tả triệu chứng, ô tìm mã ICD-10 có debounce 300ms + dropdown gợi ý (tái dùng
`SearchIcd10`/`ICD10ResultDto` đã có từ trước), nút "Lưu triệu chứng & chẩn đoán" gọi lại đúng
API `Diagnose` đã có sẵn — không viết endpoint mới. Tách phần logic tạo `OutpatientVisit`
(trước đó nằm gộp trong `OpenPrescription`) ra hàm dùng chung `EnsureVisitCreatedAsync`, vì
giờ có 2 chỗ cần visit tồn tại trước khi ghi dữ liệu (lưu chẩn đoán và mở đơn thuốc), tránh
tạo visit trùng lặp nếu bác sĩ bấm cả 2.
```
...
(vd E11 — Type 2 diabetes) trong lúc đang gọi video, lưu thành công, xác nhận dữ liệu hiện
đúng ở Billing và Hồ sơ Sức khỏe Điện tử (PHR) sau đó.
```

#### 21.4. Phần sinh viên/nhóm tự chỉnh sửa hoặc cải tiến

```text
Yêu cầu tái dùng đúng API `Diagnose`/`SearchIcd10` đã có sẵn từ luồng khám vãng lai thay vì để
AI viết endpoint riêng cho tele, để giữ nhất quán dữ liệu chẩn đoán giữa 2 luồng khám (không
tạo 2 nguồn sự thật khác nhau cho cùng 1 loại dữ liệu).
```
...
| Loại minh chứng   | Nội dung                                                                                     |
| ----------------- | --------------------------------------------------------------------------------------------- |
| Link commit       | `7bd9072` trên branch `feature/de190123-telemedicine`                                        |
| File liên quan    | `src/Mediconnect.Web/Components/Pages/Telemedicine.razor`                                     |
| Screenshot        |                                                                                                |
| Kết quả chạy/test | `dotnet build`: 0 Warning/0 Error; test sống: lưu triệu chứng + ICD-10 trong lúc gọi video thành công, dữ liệu hiện đúng ở Billing/PHR |
| Link video demo   |                                                                                                |
| Ghi chú khác      |                                                                                                |
...
```text
Tái sử dụng API/DTO đã có (SearchIcd10, Diagnose) thay vì viết mới giúp tránh phân mảnh dữ
liệu chẩn đoán giữa 2 luồng khám (vãng lai và tele) — nếu viết riêng rất dễ sinh ra 2 cách lưu
chẩn đoán khác nhau cho cùng 1 khái niệm nghiệp vụ.
```
...
| Ngày sử dụng        | 23/07/2026                                                                           |
| Công cụ AI          | Claude Code (claude-sonnet-5)                                                       |
| Mục đích sử dụng    | Fix bill thiếu phí khám (bug có sẵn của Billing, không riêng tele) + Appointment.Status không bao giờ đổi khỏi "Chờ xác nhận" |
...
```text
1. Test full flow đặt lịch → gọi video tele → chẩn đoán → tạo hoá đơn thì phát hiện hoá đơn
   không hề có phí khám. Rà `Billing.razor.SubmitCreate`: không gửi `ExamServiceId` lên API
   tạo hoá đơn dù DTO backend đã có sẵn trường này từ trước (bug có sẵn của trang Billing gốc
   — DE180526, không phải lỗi mới sinh ra từ tele, chỉ lộ rõ khi test qua tele vì trước đó
   Billing thường được test kèm dịch vụ xét nghiệm/thuốc nên vẫn ra số tiền khác 0). Thêm
   dropdown "Dịch vụ khám" lọc theo đúng khoa của visit (`VisitOption` gán thêm `ClinicId`,
   `LoadData` dựng danh sách `ExamOptions` khớp khoa), chọn xong mới gửi kèm `ExamServiceId`
   khi tạo hoá đơn.
2. Rà toàn bộ luồng nghiệp vụ hiện có: không nơi nào từng gọi update `Appointment.Status` sau
   khi khám xong — cột này đứng yên ở "Chờ xác nhận" vĩnh viễn kể từ khi tạo lịch hẹn. Thêm
   `UpdateAppointmentStatus` vào `ApiClient`, gọi trong `EndCall()` của trang tele; đồng thời
   cascade `Appointment.Status = Completed` ngay trong khối cascade `QueueTicket` có sẵn của
   `MedicalRecordService.SaveMedicalRecordAsync` (thêm `IRepository<Appointment>` vào
   constructor) để luồng walk-in/hẹn trước không qua tele cũng được cập nhật đúng, không chỉ
   riêng luồng tele.
```
...
```text
Áp dụng sau khi xác nhận `dotnet build` sạch (0 Warning/0 Error) ở cả 2 project và test sống:
tạo hoá đơn sau khám tele ra đúng phí khám cộng phí khác (nếu có); hoàn tất khám qua cả 2
luồng (tele và walk-in/hẹn trước) đều đổi đúng Appointment.Status sang Completed.
```

#### 22.4. Phần sinh viên/nhóm tự chỉnh sửa hoặc cải tiến

```text
Yêu cầu AI xác nhận rõ đây là bug có sẵn của Billing từ trước (không phải lỗi riêng của tele)
trước khi cho sửa, để tránh khoanh vùng fix chỉ trong phạm vi trang tele trong khi bug thực
tế ảnh hưởng mọi hoá đơn tạo từ trước tới giờ.
```
...
| Loại minh chứng   | Nội dung                                                                                     |
| ----------------- | --------------------------------------------------------------------------------------------- |
| Link commit       | `3f21e3f` trên branch `feature/de190123-telemedicine`                                        |
...
| Screenshot        |                                                                                                |
| Kết quả chạy/test | `dotnet build` cả 2 project (`src/mediconnect`, `src/Mediconnect.Web`): 0 Warning/0 Error; test sống: hoá đơn sau khám tele có đúng phí khám; Appointment.Status chuyển đúng Completed ở cả 2 luồng (tele + walk-in/hẹn trước) |
| Link video demo   |                                                                                                |
| Ghi chú khác      | `Billing.razor` vốn do teammate `minhPJ2812` viết — thay đổi lần này sửa logic `LoadData`/`SubmitCreate` có sẵn (không phải chỉ thêm mới). `MedicalRecordService.cs` — thay đổi thêm dependency mới vào constructor có sẵn và cascade thêm logic vào khối xử lý có sẵn (không phải chỉ thêm mới) |

#### 22.6. Nhận xét cá nhân/nhóm

```text
Cả 2 bug (thiếu phí khám, Appointment.Status không đổi) đều là lỗ hổng có sẵn từ trước, không
phải lỗi do tính năng tele gây ra — chúng chỉ tình cờ lộ ra rõ nhất khi test qua luồng tele vì
đây là lần đầu tiên toàn bộ chuỗi nghiệp vụ (đặt lịch → khám → tạo hoá đơn) được test liên tục
từ đầu đến cuối trong 1 phiên. Đây là bằng chứng cho thấy test theo luồng nghiệp vụ thật hiệu
quả hơn test từng tính năng riêng lẻ trong việc phát hiện lỗ hổng tích hợp giữa các module.
```
...
| Công cụ AI          | Claude Code (claude-sonnet-5)                                                                  |
| Mục đích sử dụng    | Đối chiếu toàn bộ đặc tả SmartClinic (DE190123) với code hiện có; audit tích hợp 4 pass (smoke chức năng, convention, hồi quy chéo module, xác minh runtime/DB) trước khi lên kế hoạch fix |
| Phần việc liên quan | Requirement / Audit (đọc code, không sinh code)                                               |
| Mức độ sử dụng      | Nghiên cứu / phân tích                                                                        |

#### 23.1. Prompt đã sử dụng

```text
[DE190123 cần bổ sung — prompt nguyên văn của phiên audit/matrix trước session hiện tại không còn trong cửa sổ hội thoại này]
```

#### 23.2. Kết quả AI gợi ý

```text
Feature completion matrix: 40 feature được đối chiếu với đặc tả, phân loại 28 hoàn thành (✅),
2 cảnh báo/chưa trọn vẹn (⚠️), 9 chưa làm (❌), 1 cần xác minh thêm (🔍).
Integration audit 4 pass:
  Pass 1 — Functional smoke: gọi thử các luồng chính, xác nhận chạy được.
  Pass 2 — Convention: đối chiếu style/pattern code với phần còn lại của dự án.
  Pass 3 — Cross-module regression: kiểm tra thay đổi SmartClinic không phá vỡ Billing/Inpatient/Queue.
  Pass 4 — Runtime/DB verification: chạy app thật + query SQL trực tiếp xác nhận hành vi khớp code.
```

#### 23.3. Phần sinh viên/nhóm đã sử dụng từ AI

```text
Dùng kết quả audit làm danh sách việc cần làm cho các session fix tiếp theo (Group A-F +
3 finding nhỏ), ưu tiên theo mức độ nghiêm trọng.
```

#### 23.4. Phần sinh viên/nhóm tự chỉnh sửa hoặc cải tiến

```text
Không áp dụng trực tiếp (session nghiên cứu thuần, không sinh code) — chỉ dùng để lên kế hoạch.
```
...
| Kết quả chạy/test |                                                |
| Link video demo   |                                                |
| Ghi chú khác      | Kết quả chi tiết 40 feature không lưu thành file riêng trong repo tại thời điểm ghi log |

#### 23.6. Nhận xét cá nhân/nhóm

```text
Đối chiếu đặc tả với code thực tế trước khi fix giúp tránh sửa nhầm phạm vi hoặc bỏ sót các
gap đã tồn tại từ session trước (ví dụ duplicate visit, allergy giả lập, billing redirect thiếu).
```
...
| Công cụ AI          | Claude Code (claude-sonnet-5)                                                                  |
| Mục đích sử dụng    | Kiểm thử end-to-end module SmartClinic sau khi fix: Flow 1-4 (31 bước), 6 edge case, 5 performance check, 3 regression check |
| Phần việc liên quan | Testing (browser thật + SQL trực tiếp)                                                        |
| Mức độ sử dụng      | Tự chạy test hoàn toàn, tự phát hiện và báo cáo bug                                            |
...
repo cũ trước khi di chuyển thư mục (xoá + rebuild); connection string appsettings.json trỏ sai
instance SQL Server (Development override đã đúng sẵn, chỉ cần set ASPNETCORE_ENVIRONMENT).
Sau khi môi trường chạy được: test qua browser thật (đăng nhập Nurse/Doctor/Patient, check-in,
gọi khám, chuyển phòng, chẩn đoán, kê đơn, tạo hoá đơn, gọi video, kết thúc cuộc gọi) + verify
trực tiếp qua SQL sau mỗi bước. Kết quả: Flow 1 (Walk-in) 5/5 PASS; Flow 2 (Appointment+
E-Prescription) 10/11 PASS (1 fail nhẹ — TD-6 stock=0 vẫn hiện trong autocomplete dù nút Thêm
bị disable đúng); Flow 3 (Telemedicine) 7 PASS + 2 SKIP (môi trường không có 2 danh tính đồng
thời + không có camera thật); Flow 4 (Edge case) 4 PASS + 1 fail (trùng nguyên nhân Flow 2) +
1 SKIP. Performance: cả 5 check đều PASS (ICD-10 ~1s, autocomplete tức thời, API ~80ms). Regression:
Billing/Inpatient/Queue đều PASS. Tổng: 0 blocking bug, 0 regression, READY FOR PR.
```

#### 25.3. Phần sinh viên/nhóm đã sử dụng từ AI

```text
Dùng toàn bộ kết quả test làm báo cáo test report chính thức, dùng 3 finding phát hiện được
(email required không đánh dấu, thiếu unique constraint DB, stock filter thiếu) làm input cho
session fix tiếp theo (Lần sử dụng AI số 26).
```

#### 25.4. Phần sinh viên/nhóm tự chỉnh sửa hoặc cải tiến

```text
- Yêu cầu AI verify mọi bước qua SQL trực tiếp (không chỉ tin UI hiển thị đúng).
- Yêu cầu AI báo cáo rõ khi không thể test được (thiếu camera thật, không thể có 2 phiên đăng
  nhập đồng thời trong cùng 1 trình duyệt) thay vì giả định kết quả.
- Từ chối để AI đăng nhập vào tài khoản bệnh nhân thật của thành viên nhóm (không phải tài khoản
  demo) khi phát hiện email trùng với email thật — chỉ dùng tài khoản demo có sẵn.
```
...
| Screenshot        |                                                                                                 |
| Kết quả chạy/test | Flow 1: 5/5 PASS; Flow 2: 10/11 PASS; Flow 3: 7 PASS + 2 SKIP; Flow 4: 4 PASS + 1 fail + 1 SKIP; Performance: 5/5 PASS; Regression: 3/3 PASS |
| Link video demo   |                                                                                                 |
| Ghi chú khác      | `.claude/launch.json` được tạo mới để chạy 2 dev server (API :5079, Web :5104) trong môi trường test |

#### 25.6. Nhận xét cá nhân/nhóm

```text
Test thật qua browser + SQL phát hiện được nhiều điểm mà brief ban đầu giả định sai (ví dụ
enum ordinal AppointmentStatus.Completed thực tế = 3 không phải 4, walk-in check-in yêu cầu
email bắt buộc nhưng brief không biết) — một lần nữa cho thấy test thực tế quan trọng hơn đọc
code suy luận.
```
